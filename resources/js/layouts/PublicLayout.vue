...
const page = usePage();
const sharedProfile = computed<{ nama_desa?: string; header_style?: string; footer_style?: string; font_style?: string; heading_font?: string } | null>(() => page.props.sharedProfile as { nama_desa?: string; header_style?: string; footer_style?: string; font_style?: string; heading_font?: string } | null);
...
const headingFont = computed(() => sharedProfile.value?.heading_font || 'inter');
...
<template>
    <div class="flex min-h-screen flex-col bg-white dark:bg-zinc-900" :class="[`font-style-${fontStyle}`, `heading-font-${headingFont}`]">
        <!-- Navbar -->
