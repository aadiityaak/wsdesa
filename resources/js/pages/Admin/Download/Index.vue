...
import { Plus, Pencil, Trash2, Download, ArrowUp } from '@lucide/vue';
...
const executeDelete = () => {
    if (deleteConfirmId.value) {
        deleteForm.delete(`/admin/download/${deleteConfirmId.value}`, {
            onSuccess: () => {
        ...
        <div class="relative mb-10 overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-50 to-white px-6 py-10 shadow-sm ring-1 ring-zinc-100 dark:from-zinc-900 dark:to-zinc-950 dark:ring-zinc-800 sm:px-10 sm:py-12">
            ...
            <div class="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white sm:text-3xl">Download</h1>
                            <p class="mt-0.5 text-sm text-zinc-500 dark:text-zinc-400">Kelola file download untuk publik</p>
                        </div>
                    </div>
                </div>
                <Button as="a" href="/admin/download/tambah" class="gap-2 rounded-full bg-rose-500 text-white shadow-sm hover:bg-rose-600">
                    <Plus class="h-4 w-4" />
                    Tambah Download
                </Button>
            </div>
            ...
            <div class="relative mt-6">
                <div class="inline-flex items-center gap-2 rounded-xl border border-zinc-100 bg-white/60 px-5 py-3 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/60">
                    ...
                    <span class="text-sm text-zinc-500 dark:text-zinc-400">File</span>
                </div>
            </div>
        </div>

        <!-- Empty state -->
        <div v-if="downloads.length === 0" class="rounded-2xl border border-zinc-100 bg-white px-6 py-16 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <Download class="mx-auto h-12 w-12 text-zinc-300 dark:text-zinc-600" />
            <p class="mt-4 text-base font-medium text-zinc-600 dark:text-zinc-400">Belum ada file download</p>
            <p class="mt-1 text-sm text-zinc-400 dark:text-zinc-500">Unggah file yang dapat diunduh oleh pengunjung.</p>
            <Button as="a" href="/admin/download/tambah" class="mt-4 gap-2 rounded-full bg-rose-500 text-white hover:bg-rose-600">
                <Plus class="h-4 w-4" />
                Tambah Download
            </Button>
        </div>
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <Button variant="ghost" size="icon-sm" as="a" :href="`/admin/download/${item.id}/edit`" class="rounded-lg">
